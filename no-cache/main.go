...
const (
	noCacheControl = "no-store, no-cache, must-revalidate, max-age=0"
...
func serve(c fiber.Ctx) error {
	...
	if strings.HasSuffix(p, "/index.html") {
		...
		c.Set("Cache-Control", noCacheControl)
	...
	h := &c.Response().Header
	h.Set("Cache-Control", noCacheControl)
...
func add(rel string, body []byte, ctype string) {
	...
	files[rel] = &file{ctype: ctype, body: body}
}
